-- SorcererTD Hub v36 (Config Fix)
-- Fixed: Config System now saves/loads correctly (Added Flags + SetFolder)
-- Logic: v35 Core (Strict Money Wait + Retry Logic + Auto Skip Color Check)
...
local TowerIDToNumber = {} 
local TowerNumberToID = {} 
local TowerInstanceMap = {} 
local TowerPositions = {} 

-- // UTILITY FUNCTIONS //
local function SafeNotify(title, desc, lifetime)
    task.spawn(function()
        pcall(function()
            Window:Notify({Title = title, Description = desc, Lifetime = lifetime or 2})
        end)
    end)
end
...
local function getSkipButton()
    local gui = LocalPlayer.PlayerGui:FindFirstChild("GameGui")
    if gui and gui:FindFirstChild("Wave") and gui.Wave:FindFirstChild("Skip") then
        return gui.Wave.Skip
    end
    return nil
end

local function isAutoSkipRed()
    local btn = getSkipButton()
    if btn and btn.Visible then
        local color = btn.BackgroundColor3
        return (color.R > 0.6 and color.G < 0.4) 
    end
    return false
end

local function isAutoSkipGreen()
    local btn = getSkipButton()
    if btn and btn.Visible then
        local color = btn.BackgroundColor3
        return (color.G > 0.6 and color.R < 0.4) 
    end
    return false
end
...
local function registerNewTower(towerInstance, forceNumber)
    ...
    if TowerInstanceMap[towerInstance] then return TowerInstanceMap[towerInstance] end
    if TowerIDToNumber[towerUUID] then return TowerIDToNumber[towerUUID] end
...
local function SaveMacro(name, data)
    local payload = {
        metadata = { recordingMode = State.RecordingMode, createdAt = os.time(), version = "2.2" },
        actions = data
    }
            ...
            if Root then
                ...
                if TpObj then
                    ...
                    if TpObj:FindFirstChild("Teleports") then
                        if TpObj.Teleports:FindFirstChild("Entrance") then targetPart = TpObj.Teleports.Entrance
                        elseif TpObj.Teleports:FindFirstChild("TeleportIn") then targetPart = TpObj.Teleports.TeleportIn end
                    end
                    ...
                    if targetPart then
                        ...
                        if IsSukuna then ReplicatedStorage.Remotes.SukunaTeleporters.ChooseStage:FireServer(TpObj, false)
                        else ReplicatedStorage.Remotes.Teleporters.ChooseStage:FireServer(TpObj, State.SelectedStage, State.SelectedDifficulty, false) end
                        
                        task.wait(1)
                        if State.AutoStart then
                            SafeNotify("Auto Join", "Starting Game...")
                            if IsSukuna then ReplicatedStorage.Remotes.SukunaTeleporters.Start:FireServer(TpObj)
                            else ReplicatedStorage.Remotes.Teleporters.Start:FireServer(TpObj) end
...
mt.__namecall = function(self, ...)
    ...
    if State.IsRecording and (method == "InvokeServer" or method == "FireServer") then
        ...
        if remoteName == "RequestTower" then State.LastRequestedUnit = args[1] end
        ...
        if remoteName == "SpawnNewTower" then
            task.spawn(function()
                ...
                local recordTime = (State.RecordingMode == "Hybrid") and (os.time() - State.GameStart) or 0
                    ...
                    for _, t in pairs(Workspace.Towers:GetChildren()) do
                        if t.PrimaryPart and (t.PrimaryPart.Position - PositionCFrame.Position).Magnitude < 1.0 then
                            if not TowerInstanceMap[t] then
                                registerNewTower(t, towerNumber)
                                return
                            end
                        end
                    end
        ...
        if remoteName == "UpgradeTower" then
            task.spawn(function()
                ...
                if towerNumber then
                    ...
                    local recordTime = (State.RecordingMode == "Hybrid") and (os.time() - State.GameStart) or 0
                    table.insert(State.CurrentMacro, { Type = "Upgrade", Time = recordTime, TowerNumber = towerNumber, Cost = Cost, Wave = GetWave() })
        ...
        if remoteName == "SellTower" then
            ...
            task.spawn(function()
                if towerNumber then
                    local recordTime = (State.RecordingMode == "Hybrid") and (os.time() - State.GameStart) or 0
                    table.insert(State.CurrentMacro, { Type = "Sell", Time = recordTime, TowerNumber = towerNumber, Wave = GetWave() })
        ...
        if remoteName == "ChangeTowerMode" or remoteName:find("Domain") or remoteName == "SukunaDomain" or remoteName == "GojoDomain" or remoteName == "MahitoDomain" then
            task.spawn(function()
                ...
                if towerNumber then
                    local recordTime = (State.RecordingMode == "Hybrid") and (os.time() - State.GameStart) or 0
                    table.insert(State.CurrentMacro, { Type = "Ability", Time = recordTime, Action = remoteName, TowerNumber = towerNumber, Wave = GetWave() })
        ...
        if IsInLobby() then
            ...
            for i, Action in ipairs(Actions) do
                ...
                if Action.Type == "Place" then
                    ...
                    if Action.UnitName then ReplicatedStorage.Functions.RequestTower:InvokeServer(Action.UnitName) task.wait(0.1) end
                ...
                elseif Action.Type == "Upgrade" then
                    ...
                    while not upgraded and State.IsReplaying do
                        local t = FindTowerByNumber(Action.TowerNumber)
                        if t then 
                            local moneyBefore = GetMoney()
                            ReplicatedStorage.Functions.UpgradeTower:InvokeServer(t)
                            task.wait(0.5)
                            if GetMoney() < moneyBefore then upgraded = true else task.wait(0.5) end
                        else
                            SafeNotify("Debug", "Upgrade: Tower #"..Action.TowerNumber.." missing, waiting...")
                            task.wait(1)
                        end
                    end
                ...
                elseif Action.Type == "Ability" then
                    local t = FindTowerByNumber(Action.TowerNumber)
                    if t then 
                        if Action.Action == "ChangeTowerMode" then ReplicatedStorage.Functions.ChangeTowerMode:InvokeServer(t)
                        else 
...
Window:Dialog({
    ...
    Buttons = {
        { Name = "Mobile", Callback = function() Window:SetScale(0.65) end },
        { Name = "PC", Callback = function() Window:SetScale(1) end },
    }
})

-- MACRO RECORDER
sections.MacroRecorder:Header({ Name = "Macro Recorder" })
sections.MacroRecorder:Input({ Name = "Macro Name", Placeholder = "Name", Callback = function(v) State.MacroName = v end }, "MacroNameInput")
sections.MacroRecorder:Dropdown({ Name = "Macro Type", Options = {"Index", "Hybrid"}, Default = "Index", Callback = function(v) State.RecordingMode = v end }, "MacroTypeDropdown")
sections.MacroRecorder:Toggle({ Name = "Record Macro", Default = false, Callback = function(v)
    if v then
        if State.MacroName == "" then SafeNotify("Error", "Enter name first!") return end
...
local MacroDrop = sections.ManageMacro:Dropdown({ Name = "Select to Play", Options = RefreshMacroList(), Default = "nil", Callback = function(v) State.SelectedMacroToPlay = v end }, "PlaySelectDropdown")
sections.ManageMacro:Toggle({ Name = "Play Loop", Default = false, Callback = function(v) State.IsReplaying = v if v then task.spawn(PlayMacroLoop) end end }, "PlayLoopToggle")
local DeleteDrop = sections.ManageMacro:Dropdown({ Name = "Select to Delete", Options = RefreshMacroList(), Default = "Select", Callback = function(v) State.SelectedMacroToDelete = v end }, "DeleteSelectDropdown")
sections.ManageMacro:Button({ Name = "Refresh Lists", Callback = function() 
    local list = RefreshMacroList()
    MacroDrop:ClearOptions() MacroDrop:InsertOptions(list)
    DeleteDrop:ClearOptions() DeleteDrop:InsertOptions(list)
end })
sections.ManageMacro:Button({ Name = "Delete Macro", Callback = function() 
    if State.SelectedMacroToDelete then delfile(FolderPath.."/"..State.SelectedMacroToDelete..".json") end 
end })

-- MAIN UI
sections.GameControls:Header({ Name = "Story Mode Auto Join" })
local stagesList = {} for i = 1, 15 do table.insert(stagesList, "Stage " .. i) end
sections.GameControls:Dropdown({ Name = "Select Mode", Options = {"Story", "Sukuna"}, Default = "Story", Callback = function(v) State.SelectedMode = v end }, "SelectModeDropdown")
sections.GameControls:Dropdown({ Name = "Select Stage", Options = stagesList, Default = "Stage 1", Callback = function(v) State.SelectedStage = tonumber(v:match("%d+")) end }, "SelectStageDropdown")
sections.GameControls:Dropdown({ Name = "Select Difficulty", Options = {"Normal", "HellMode"}, Default = "Normal", Callback = function(v) State.SelectedDifficulty = v end }, "SelectDifficultyDropdown")
sections.GameControls:Toggle({ Name = "Auto Join Story", Default = false, Callback = function(v) State.AutoJoin = v if v then task.spawn(RunAutoJoin) end end }, "AutoJoinToggle")
sections.AutoFeatures:Toggle({ Name = "Auto Start Game", Default = false, Callback = function(v) State.AutoStart = v end }, "AutoStartToggle")

sections.GameControls:Header({ Name = "Controls" })

sections.GameControls:Toggle({
    Name = "Auto Skip",
    Default = false,
    Callback = function(state)
        State.AutoSkip = state
        if state then
            task.spawn(function()
                while State.AutoSkip do
                    if IsInGame() then
                        if isAutoSkipRed() then
                            ReplicatedStorage.Remotes.AutoSkip:FireServer()
                        elseif isAutoSkipGreen() then
                            task.wait(2) 
                        end
                    end
                    task.wait(0.5)
...
sections.AutoFeatures:Toggle({ Name = "Auto Replay", Default = false, Callback = function(v) State.AutoReplay = v if v then task.spawn(function() while State.AutoReplay do if IsInGame() then local gui = LocalPlayer.PlayerGui:FindFirstChild("GameGui") if gui and gui:FindFirstChild("EndScreen") and gui.EndScreen.Visible then ReplicatedStorage.Events.Replay:FireServer() task.wait(5) end end task.wait(1) end end) end end }, "AutoReplayToggle")

MacLib:SetFolder("Crazy_Hub")
